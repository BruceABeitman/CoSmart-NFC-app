...
public class NfcService extends Application implements DeviceHostListener {
    ...
    class EnableDisableTask extends AsyncTask<Integer, Void, Void> {
        ...
        boolean enableInternal() {
            ...
            if (false) {  //nate1-removed this !mDeviceHost.initialize() r/w false
        ...
        synchronized (this) {
            ...
            if (mScreenState >= POLLING_MODE) {
                if (force || !mNfcPollingEnabled) {
                    ...
                    Log.i(TAG, "Something should at least happen here");
                }
            } else {
                if (force || mNfcPollingEnabled) {
                    Log.d(TAG, "NFC-C OFF");
    ...
    final class NfcServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) { //nate-3 removed this msg.what r/w MSG_NDEF_TAG
                case MSG_MOCK_NDEF: {
                    ...
                    if (delivered) {
			Log.w(TAG, "Delivered message to tag: " + tag);
                        playSound(SOUND_END);
                    } else {
        ...
        private boolean llcpActivated(NfcDepEndpoint device) {
            ...
            String s = Integer.toString(device.getMode());
            Log.i(TAG, s);

            if (false) { //nate-device.getMode() == NfcDepEndpoint.MODE_P2P_TARGET
            ...
            } else if (true) {//nate-device.getMode() == NfcDepEndpoint.MODE_P2P_INITIATOR
                if (DBG) Log.d(TAG, "NativeP2pDevice.MODE_P2P_INITIATOR");
                /* Check LLCP compliancy */
                if (true) {//nate-mDeviceHost.doCheckLlcp()
                    /* Activate LLCP Link */
                    if (true) {//nate-mDeviceHost.doActivateLlcp()
