...
public class P2pLinkManager implements Handler.Callback, P2pEventListener.Callback {
    ...
    static final boolean mStateMig = false;
    ...
    public void setNdefToSend(NdefMessage staticNdef, INdefPushCallback callbackNdef) {
        synchronized (this) {
	    Log.i(TAG, "settingNdefToSend: " + staticNdef);
	    Log.i(TAG, "Ndef from callback: " + callbackNdef);
        ...
        synchronized (P2pLinkManager.this) {
            switch (mLinkState) {
                case LINK_STATE_DOWN:
                    ...
                    if (mMessageToSend != null) {//nate-mMessageToSend != null
    ...
    void prepareMessageToSend() {
	    ...
	    //^ Get all running application's messages
	    if (mStateMig) {
		List<RunningTaskInfo> tasks = mActivityManager.getRunningTasks(5);
		NdefRecord[] Records = new NdefRecord[tasks.size()*2];
		String pkg = "";
		String tpkg = "";
		int j = 0;
		for (int i = 0; i < tasks.size()*2-1; i++) {
		    tpkg = tasks.get(j).baseActivity.getPackageName();
		    try {
                	ApplicationInfo appInfo = mPackageManager.getApplicationInfo(tpkg, 0);
			//pkg = pkg + tpkg.length() + tpkg;

			String appU = "http://market.android.com/search?q=pname:" + tpkg;
			NdefRecord appUri = NdefRecord.createUri(Uri.parse(appU));
		        NdefRecord appRecord = NdefRecord.createApplicationRecord(tpkg);
			...
			j++;
                	...
                	Log.e(TAG, "Bad package returned from ActivityManager: " + tpkg);
    		    }
        	}


		// Return the appUri (URI, with all apps concatenated, and last apps package
		mMessageToSend = new NdefMessage(Records);


		byte[] bytes = mMessageToSend.toByteArray();
		    ...
		    if (callback != null) {
			    ...
			    Log.i(TAG, "Created app's Ndef");
		        } catch (RemoteException e) {
		            // Ignore
		        }
		    }

		    if (messageToSend == null) {//nate-messageToSend == null
		        messageToSend = createDefaultNdef();
		        Log.i(TAG, "Created default Ndef");                
		    }
    ...
    NdefMessage createDefaultNdef() {
        ...
        if (tasks.size() > 0) {
            ...
            try {
                ...
                String s1 = Integer.toString(ApplicationInfo.FLAG_SYSTEM);
                Log.i(TAG, s);
                Log.i(TAG, s1); 
                if (true) {//nate- 0 == (appInfo.flags & ApplicationInfo.FLAG_SYSTEM)
	...
	private void setMobileDataEnabled(Context context, boolean enabled) {
	   ...
	   */
	    ...
	    if (DBG) Log.d(TAG, "WifiState: " + wifiManager.getWifiState());
	    //WifiLock wifiLock = wifiManager.createWifiLock(WifiManager.WIFI_STATE_DISABLED, "MyWifiLock");
	    //wifiLock.acquire();
	    //wifiLock.setReferenceCounted(false);

	    //ConnectivityManager cm = (ConnectivityManager) context.getSystemService(Context.CONNECTIVITY_SERVICE);
	    //if (DBG) Log.d(TAG, "Connectivity WiFiInfo: " + cm.getNetworkInfo(ConnectivityManager.TYPE_WIFI).isConnectedOrConnecting());
	...
	*/
    ...
    final NdefPushServer.Callback mNppCallback = new NdefPushServer.Callback() {
	    ...
	    if (DBG) Log.d(TAG, "NdefPushServer.Callback: " + msg);            
    ...
    void onReceiveComplete(NdefMessage msg) {
        ...
        // Make callbacks on UI thread
	if (DBG) Log.d(TAG, "onReceiveComplete: " + msg);
    ...
    public boolean handleMessage(Message msg) {
		...
		if (DBG) Log.d(TAG, "handleMessage MSG_RECEIVE_COMPLETE: " + msg);
                NdefMessage m = (NdefMessage) msg.obj;
                synchronized (this) {
                    if (mLinkState == LINK_STATE_DOWN) {
			if (DBG) Log.d(TAG, "mLinkState equaled LINK_STATE_DOWN");
                        break;
                    }
                    if (mSendState == SEND_STATE_SENDING) {
			if (DBG) Log.d(TAG, "mSendState equaled SEND_STATE_SENDING");
                        cancelSendNdefMessage();
                    }
                    mSendState = SEND_STATE_NOTHING_TO_SEND;
